...
function App() {
  ...
  const currentStep = currentView === 'results' ? (activeTab === 'overview' ? 3 : 4) : busy ? 2 : 1

  return (
    <div className="app-shell">
      <AppHeader currentView={currentView} hasAnalysis={Boolean(analysis)} menuOpen={menuOpen} onMenuToggle={() => setMenuOpen((open) => !open)} onNavigate={navigate} serviceStatus={serviceStatus} />
      <main className="app-content" id="app-content">
        {currentView === 'analyze' ? <ProgressSteps currentStep={currentStep} /> : null}
        {currentView === 'analyze' ? (
          <UploadView
      ...
      <footer className="app-footer"><span>Resume Lens</span><p>Scores support review; they do not predict hiring decisions.</p><a href="https://github.com/rammohanrediee/AI-Resume-Analyzer" target="_blank" rel="noreferrer">Source</a></footer>
      {currentView === 'results' ? (
        <nav className="mobile-bottom-nav" aria-label="Mobile workspace">
          <button type="button" onClick={() => navigate('analyze')}><Icon name="upload" size={20} /><span>Analyze</span></button>
          <button className="mobile-bottom-nav__active" type="button" aria-current="page" onClick={() => navigate('results')}><Icon name="chart" size={20} /><span>Results</span></button>
        </nav>
      ) : null}
    </div>
  )
